...
class AnalyticBeam:
    ...
    def interp(self, az_array, za_array, freq_array, reuse_spline=None, spline_opts=None):
        ...
        Parameters
        ----------
        az_array: array of float
            azimuth values in radians. Same shape as za_array.
            The azimuth is defined under the UVBeam convention: North of East(East=0, North=pi/2)
        za_array: array of float
            zenith angle values in radians. Same shape as az_array.
        ...
        Returns
        -------
        beam_vals: array of float
            An array of beam values.
            Let Ncomponents = az_array.size = za_array.size.
            Let Nfreqs = freq_array.size
                If self.beam_type == 'efield', then the returned shape
                    (2, 1, 2, Nfreqs, Ncomponents)
                if self.beam_type == 'power', then the returned shape is
                    (1, 1, 4, Nfreqs, Ncomponents)
